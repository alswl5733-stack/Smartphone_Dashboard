    ...
    try:
        ...
        for i, item in enumerate(items[:200]):
           ...
           # 💡 [핵심] '특정 국가 지각 출시(재탕)'를 걸러내고 '글로벌 최초 공개'만 잡는 똑똑해진 프롬프트 유지
            ...
            [유연한 판별 기준]
            1. '출시 행사를 시청하는 법(How to watch)', '오늘 글로벌 런칭 이벤트(Global launch event today)' 등은 제품이 오늘 공개된다는 팩트이므로 '예'로 인정합니다.
            2. [지각 출시 필터링] 과거에 이미 출시된 폰이 특정 국가에 뒤늦게 런칭하는 기사는 여전히 '아니오'입니다.
            3. '예정(Expected)', '유출(Leak)', '루머(Rumor)'는 '아니오'입니다.
            4. 기사 내용이 제품의 공식적인 최초 등장/발표를 다룬다면 인정하세요.
            위 기준에 미달하면 아니오라고 답하고, 신제품이 맞다면 모델명만 정확히 적으세요. '예'나 다른 문장은 절대 포함하지 마세요. 오직 결과값(모델명)만 출력하세요.
            """
